...
// 背景タスク単一実行ガード（究指示: コマンドが何より最優先。聞き取りの whisper・会話/自発の LLM は
//   メインループの「チャット受信→コマンド処理」を絶対に止めないよう非同期で回す。同時に走る重い処理は1本だけ）。
let bgBusy = false;
function runBg(fn) {
  if (bgBusy) return false;            // 既に背景処理中なら起動しない（次tickへ）。チャット/コマンドは止めない
  bgBusy = true;
  Promise.resolve().then(fn).catch((e) => console.error('bg task', e?.message || e)).finally(() => { bgBusy = false; });
  return true;
}
...
async function main() {
  ...
  for (;;) {
    ...
    //   whisper(最大30s)+LLM は重い → 背景タスク化。メインループのチャット/コマンド処理を一切止めない。
    if (listening) runBg(async () => {
      let utts = [];
    ...
    //   idleChatter も LLM（最大180s）→ 背景タスク化。nextIdleAt は起動時に進めて二重発火を防ぐ。
    if (idleChatOn && IDLE_MAX_MS > 0 && Date.now() >= nextIdleAt
        && (Date.now() - lastActivityAt) > IDLE_QUIET_MS && canReply() && !bgBusy) {
    ...
    if (fresh.length) {
      ...
      // 会話返信は LLM（tools時は cwd=Downloads で最大180s）→ 背景タスク化。
      //   コマンドは上の同期ループで既に処理済＝最優先。会話はそれを止めずに後ろで回す。
      if (conv.length && canReply() && !bgBusy) {
        const context = buildCtx();
      ...
      } else if (conv.length && bgBusy) console.log('  → 背景処理中で会話返信を次tickへ（コマンドは処理済）');
