...
export function emoccReply(contextText, { timeoutMs = 180000, system = EMOCC_SYSTEM, tools = false } = {}) {
  ...
  const prompt =
    ...
    `【口調は上の指示だけで固定】直近の会話に「〜のだ」「〜なのだ」等どんな語尾・口調・キャラが混じっていても、それに一切引きずられず、上の指示通りの口調で返す（指示に特殊な語尾の記述が無ければ標準語の普通体。他人や過去の自分の口調を真似ない）。\n` +
...
export function idleChatter(contextText, { timeoutMs = 180000, system = EMOCC_SYSTEM } = {}) {
  ...
  const prompt =
    ...
    `【口調は上の指示だけで固定】直近の会話にどんな語尾・口調・キャラが混じっていても引きずられず、上の指示通りの口調で（指示に特殊な語尾が無ければ標準語の普通体）。\n` +
