...
使い方（node 側から shell out する想定。出力は最終行に JSON 1 行）:
  ...
  python3 yay_api.py members <call_id>      # 通話の参加者名簿（入退室ジングル用）
...
TOKEN_FILE = os.path.join(HERE, ".yay_token")
DEVICE_FILE = os.path.join(HERE, ".yay_device")
...
async def cmd_members(client, call_id: int) -> dict:
    """通話の参加者名簿（入退室ジングル用）。{id, nickname, uuid} の軽量リストを返す。

    Agora の uid(conference_call_user_uuid) は不透明なので名前に紐付かない。
    代わりに通話の conference_call_users（Yay user 一覧）を取り、bot 側で差分を取って
    「○○いらっしゃい/またね」を出す。返すのは挨拶に要る最小フィールドだけ。
    """
...
async def main(argv):
    if not argv:
        _emit({"ok": False, "error": "usage: yay_api.py <check|active|creds|leave> [arg]"})
        return 2
    cmd = argv[0]
    client = None
    try:
