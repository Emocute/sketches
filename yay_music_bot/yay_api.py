...
使い方（node 側から shell out する想定。出力は最終行に JSON 1 行）:
  ...
  python3 yay_api.py join <conference_id>   # Yay側の通話参加し直し（名簿落ち復帰用）
...
async def cmd_join(client, conference_id: int) -> dict:
    """Yay 側の通話参加（start_conference_call）。
    Yay はしばらくすると bot の参加を名簿から落とすことがあり、その間アプリ上で bot が
    見えず chat も表示されない。bot 側の見張りがここを叩いて参加し直す（Agora 接続は別物で生きたまま）。"""
    res = await client.start_conference_call(conference_id=conference_id)
    conf = getattr(res, "conference_call", None)
    return {"ok": conf is not None, "stage": "join", "conference_id": conference_id}
...
async def main(argv):
    if not argv:
        _emit({"ok": False, "error": "usage: yay_api.py <check|active|creds|join|members|leave> [arg]"})
        return 2
    cmd = argv[0]
    client = None
    try:
        ...
        if cmd == "check":
            ...
            if len(argv) < 2:
                out = {"ok": False, "error": "join は conference_id が必要"}
            else:
                out = await cmd_join(client, int(argv[1]))
        elif cmd == "members":
            if len(argv) < 2:
